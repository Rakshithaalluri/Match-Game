...
class App extends Component {
  ...
  componentDidMount() {
    this.countdown = setInterval(this.statusChange, 1000)
  }
  ...
  getFilteredItems = () => {
    ...
    const filteredProjects = imagesList.filter(
      eachThumbnailItem => eachThumbnailItem.category === activeTabId,
    )
    return filteredProjects
  }

  render() {
    const {activeTabId, seconds, score, isTrue, imgUrl} = this.state
    const filteredProjects = this.getFilteredItems()

    return (
      <>
        <nav className="navbar">
          <div className="navbar-score-time">
            <img
              src="https://assets.ccbp.in/frontend/react-js/match-game-website-logo.png"
              alt="website logo"
              className="website-logo"
            />
            <ul className="scores-list-container">
        ...
        <div className="bg-match-container">
          {!isTrue && (
            <div className="game-container">
              <img src={imgUrl} alt="match" className="first-object-image" />
              ...
              <ul className="thumbnails-container">
                {filteredProjects.map(eachThumbnail => (
                  <ThumbnailItem
                    key={eachThumbnail.id}
                    thumbnailDetails={eachThumbnail}
                    clickThumbnailItem={() =>
                      this.clickThumbnailItem(eachThumbnail.thumbnailUrl)
                    }
                  />
                ))}
              </ul>
